...
static const struct event_reg {
	...
	enum {
		GEN3_PRE_MR4	= BIT(SWITCHTEC_EVLIST_VER_GEN3_PRE_MR4),
		GEN3_MR4	= BIT(SWITCHTEC_EVLIST_VER_GEN3_MR4),
		ALL_GEN3	= GEN3_MR4 | GEN3_PRE_MR4,
		GEN3_PAX	= BIT(SWITCHTEC_EVLIST_VER_GEN3_PAX),
		EVLIST_V1	= BIT(SWITCHTEC_EVLIST_VER_V1),
		ALL_GEN4	= EVLIST_V1,
		ALL_VERS	= (ALL_GEN3 | GEN3_PAX | ALL_GEN4),
	} ver_mask;
} event_regs[] = {
	EV_GLB(SWITCHTEC_IOCTL_EVENT_STACK_ERROR, stack_error_event_hdr,
	       ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_PPU_ERROR, ppu_error_event_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_ISP_ERROR, isp_error_event_hdr,
	       ALL_VERS & ~GEN3_MR4),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_SYS_RESET, sys_reset_event_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_FW_EXC, fw_exception_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_FW_NMI, fw_nmi_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_FW_NON_FATAL, fw_non_fatal_hdr,
	       ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_FW_FATAL, fw_fatal_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_TWI_MRPC_COMP, twi_mrpc_comp_hdr,
	       ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_TWI_MRPC_COMP_ASYNC,
	       twi_mrpc_comp_async_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_CLI_MRPC_COMP, cli_mrpc_comp_hdr,
	       ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_CLI_MRPC_COMP_ASYNC,
	       cli_mrpc_comp_async_hdr, ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_GPIO_INT, gpio_interrupt_hdr,
	       ALL_VERS),
	EV_GLB(SWITCHTEC_IOCTL_EVENT_GFMS, gfms_event_hdr,
	       GEN3_PAX),
	EV_PAR(SWITCHTEC_IOCTL_EVENT_PART_RESET, part_reset_hdr, ALL_VERS),
	EV_PAR(SWITCHTEC_IOCTL_EVENT_MRPC_COMP, mrpc_comp_hdr, ALL_VERS),
	...
	EV_PAR(SWITCHTEC_IOCTL_EVENT_INTERCOMM_REQ_NOTIFY, intercomm_notify_hdr,
	       GEN3_PAX),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_AER_IN_P2P, aer_in_p2p_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_AER_IN_VEP, aer_in_vep_hdr,
	       ALL_VERS & ~GEN3_MR4),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_DPC, dpc_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_CTS, cts_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_HOTPLUG, hotplug_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_IER, ier_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_THRESH, threshold_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_POWER_MGMT, power_mgmt_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_TLP_THROTTLING, tlp_throttling_hdr,
	       ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_FORCE_SPEED, force_speed_hdr, ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_CREDIT_TIMEOUT, credit_timeout_hdr,
	       ALL_VERS),
	EV_PFF(SWITCHTEC_IOCTL_EVENT_LINK_STATE, link_state_hdr, ALL_VERS),
};
	...
	struct switchtec_ioctl_event_ctl __user *uctl)
{
	...
	if (!(event_regs[ctl.event_id].ver_mask & BIT(stdev->evlist_ver)))
		return -EINVAL;
...
static irqreturn_t switchtec_event_isr(int irq, void *dev)
{
	...
	for (eid = 0; eid < SWITCHTEC_IOCTL_MAX_EVENTS; eid++)
		if (event_regs[eid].ver_mask & BIT(stdev->evlist_ver))
...
static enum switchtec_rel get_release(struct switchtec_dev *stdev)
{
	...
	if (stdev->gen == SWITCHTEC_GEN3) {
		if (fw_ver < 0x10b0080)
			return SWITCHTEC_REL_GEN3_PRE_MR4;
		else if (fw_ver < 0x2060000)
			return SWITCHTEC_REL_GEN3_MR4;
		else
...
static const struct pci_device_id switchtec_pci_tbl[] = {
	...
	SWITCHTEC_PCI_DEVICE(0x4000, SWITCHTEC_GEN4),  //PFXI 100XG4
	SWITCHTEC_PCI_DEVICE(0x4084, SWITCHTEC_GEN4),  //PFXI 84XG4
	SWITCHTEC_PCI_DEVICE(0x4068, SWITCHTEC_GEN4),  //PFXI 68XG4
	SWITCHTEC_PCI_DEVICE(0x4052, SWITCHTEC_GEN4),  //PFXI 52XG4
	SWITCHTEC_PCI_DEVICE(0x4036, SWITCHTEC_GEN4),  //PFXI 36XG4
	SWITCHTEC_PCI_DEVICE(0x4028, SWITCHTEC_GEN4),  //PFXI 28XG4
	SWITCHTEC_PCI_DEVICE(0x4100, SWITCHTEC_GEN4),  //PSXI 100XG4
	SWITCHTEC_PCI_DEVICE(0x4184, SWITCHTEC_GEN4),  //PSXI 84XG4
	SWITCHTEC_PCI_DEVICE(0x4168, SWITCHTEC_GEN4),  //PSXI 68XG4
	SWITCHTEC_PCI_DEVICE(0x4152, SWITCHTEC_GEN4),  //PSXI 52XG4
	SWITCHTEC_PCI_DEVICE(0x4136, SWITCHTEC_GEN4),  //PSXI 36XG4
	SWITCHTEC_PCI_DEVICE(0x4128, SWITCHTEC_GEN4),  //PSXI 28XG4
	{0}
};
